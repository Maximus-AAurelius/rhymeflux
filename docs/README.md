...
- **PACKS tab — split a whole song into sound packs**: pick a file from SONGS and tap **QUICK SPLIT** (vocals + instrumental, fast) or **FULL SPLIT** (vocals, drums, bass, guitar, piano, other, plus a computed instrumental — guitar/piano are the least reliable of the six, Demucs itself calls that pair experimental). Neither button runs anything in the browser — they queue a job that a background helper on your computer picks up automatically:
  1. One-time setup, from the repo root: `pip install demucs supabase soundfile numpy`.
  2. Start the helper: `python scripts\split_worker.py`. It asks for your Rhymeflux email/password once, then sits there watching for jobs — leave the terminal window open (or set it to run at login if you want it always available).
  3. Back in the app, tap QUICK SPLIT or FULL SPLIT. The row shows live status ("Splitting on your computer…", then "Done"), and the result shows up under **YOUR SOUND PACKS** automatically — no need to touch the terminal again after the helper's running.
  Every split stem gets a play button and eight pad-number buttons — tapping a number loads that piece straight onto a BEATS pad, and it now actually persists to your account (a `pads` table), so it's still there next time you open the app on any device. (`scripts\split_stems.py` still exists too, for a one-off manual split by file path without running the helper.)
- **STEMS panel (FLOW tab)** still just displays and plays whatever's been split for the active beat — the splitting itself happens via PACKS (above).
