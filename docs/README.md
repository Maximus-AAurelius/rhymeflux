...
- **BEATS tab**: an 8-pad sampler. LOAD a sound onto each pad, tap to trigger, or play them live from a connected MIDI keyboard — notes 36–43 map to pads 1–8 (standard drum-rack convention). A 16-step sequencer per pad runs on accurate Web Audio timing with its own BPM control, so you can build and loop a pattern. Pad sounds are local to the browser session for now — not yet saved to your account.
